...
#
# Currently this program can only analyzxxe C/C++ code.
#
...
def initialize_ruleset():
    ...
    if showheading:
        print("Number of rules (primarily dangerous function names) in C/C++/JAVA ruleset:", len(
            c_ruleset))
        if output_format:
            print("<p>")
    if list_rules:
...
def display_header():
    ...
    if not displayed_header:
        if output_format:
            ...
            print("Edited by %s, Hackveda Limited." % version)
        displayed_header = 1
